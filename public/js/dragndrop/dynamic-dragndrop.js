$(function () {
	...
	$body.on("click", ".dnd-plus-button", function () {
		...
		$(sourceContainer).data(
			"layout-template",
			"MelisFront/dnd-2-cols-down-tpl"
		);
		...
		if (!row.length) {
			...
			if (!existingContent.hasClass("melis-dragdropzone-container")) {
				...
				let wrapper = $(
					'<div class="melis-dragdropzone-container"></div>'
				).attr({
		...
		if (!wrapperToClone.length) {
			console.warn(
				"No wrapper with data-dragdropzone-id='" + sourceId + "' found."
			);
			return;
		}
		...
		clonedWrapper.find("[data-plugin-id]").each(function () {
			...
			if (match) {
				...
				if ($(this).parent().hasClass("melis-ui-outlined")) {
					...
					let str = $(this)
						.parent()
						.children(".plugin-hardcoded-conf")
						.text()
						.trim();
