$(function () {
	...
	$body
		.on(
			"mouseenter",
			".melis-dragdropzone-container > .dnd-layout-wrapper",
			function () {
	...
	$body
		.on(
			"mouseenter",
			".melis-dragdropzone-container > .dnd-layout-wrapper > .dnd-layout-buttons",
			function () {
	...
	$body.on("click", ".dnd-layout-buttons div[data-dnd-tpl]", function () {
		...
		$(this)
			...
			.prepend(tempLoader.removeClass("hidden"));
							...
							if (!pluginExist.length) {
								...
								$.each(dndContents, (di, dv) => {
									...
									if ($(dv).text().trim() !== "") {
										...
										let pluginId = $(v)
											.find(".melis-plugin-tools-box")
											.data("pluginId");
										let plugin = newLayout.find(
											"[data-plugin-id='" + pluginId + "']"
										);

										// avoid plugins that already render in the new layout from backend
										if (!plugin.length)
											$(v).appendTo(
												newLayout.find(".melis-dragdropzone:first")
											);
									}
								});
							}
	...
	$body.on("click", ".dnd-plus-button", function () {
		...
		if (pluginReferer) dndId = pluginReferer;
	...
	$body.on("click", ".dnd-remove-button", function () {
		...
		let dnds = dndContainer
			.parents(".dnd-plugins-content")
			.find("> div > div > div.melis-dragdropzone-container");
