...
var melisCms = (function() {
	...
	function savePage(idPage) {
		var pageNumber =
				typeof idPage === "string" ? idPage : $(this).data("pagenumber"),
			fatherPageId = $(this).data("fatherpageid");

		// convert the serialized form values into an array
		var datastring = $(
			"#" + pageNumber + "_id_meliscms_page form"
		).serializeArray();

		if ($("#" + pageNumber + "_id_page_taxonomy").length) {
			var pageTags = $("#" + pageNumber + "_id_page_taxonomy")
				.data("tags")
				.toString();
		...
		$.ajax({
			type: "POST",
			url:
				"/melis/MelisCms/Page/savePage?idPage=" +
				pageNumber +
				"&fatherPageId=" +
				fatherPageId,
			data: datastring,
			...
			.done(function(data) {
				if (data.success === 1) {
					...
					if (pageCreationId === "0_id_meliscms_page") {
						...
						var newPageZoneId =
							data.datas.idPage +
							pageCreationId.substring(1, pageCreationId.length);
						var item_icon =
							data.datas.item_icon !== "" ? data.datas.item_icon : "fa-file-o";
						...
						melisHelper.tabOpen(
							...
							item_icon,
							newPageZoneId,
							data.datas.item_melisKey,
							{ idPage: data.datas.idPage }
						);
					} else {
		...
		$.ajax({
			...
			.done(function(data) {
				if (data.success === 1) {
					...
					$(
						'.page-publishunpublish[data-pagenumber="' + pageNumber + '"]'
					...
					$("#" + data.datas.item_zoneid + " .page-title h1:not('span')").text(
						data.datas.item_name
					);

					$(
						".tabsbar a[data-id='" +
							data.datas.item_zoneid +
							"'] .navtab-pagename"
					).text(pageNumber + " - " + data.datas.item_name);

					// hide the saved version text of the page once its published
					$(
						"#" + data.datas.item_zoneid + " .page-title .saved-version-notif"
					).fadeOut();
	...
	function refreshTreeview(pageNumber, self) {
		...
		$.ajax({
			url:
				"/melis/MelisCms/TreeSites/getPageIdBreadcrumb?idPage=" +
				pageNumber +
				"&includeSelf=" +
				optionalArg,
			encode: true,
					...
					.done(function() {
						tree
							...
							.done(function() {
								...
								if (
									$(
										"#id-mod-menu-dynatree .ui-fancytree > li:last-child"
									).hasClass("fancytree-lastsib") === false
								) {
									$(
										"#id-mod-menu-dynatree .ui-fancytree > li:last-child"
									).remove();
	...
	function displaySettings() {
		...
		$(this)
			.parents(".page-head-container")
			.next(".page-content-container")
			.find(".melis-iframe")
			.animate(
				{
					width: displayWidth,
	...
	function cmsTabEvents() {
		...
		$("form[name='pageseo'] input[textarea='pseo_meta_description']").trigger(
			"keyup"
		);
	...
	function disableCmsButtons(id) {
		...
		$("#" + id + "_id_meliscms_page_action_tabs")
			.addClass("relative")
	...
	$body.on(
		...
		"form[name='pageseo'] input[name='pseo_meta_title']",
		{ limit: 70 },
		charCounter
	);
	...
	$body.on(
		"keyup keydown change",
		"form[name='pageseo'] textarea[name='pseo_meta_description']",
		{ limit: 160 },
		charCounter
	);
	...
	$body.on(
		"shown.bs.tab",
		".page-content-container .widget-head.nav ul li a",
		cmsTabEvents
	);
	...
	$body.on(
		"click",
		".page-content-container .widget-head.nav ul li a.history, .page-content-container .widget-head.nav ul li a.more_windows",
