$(function () {
	...
	$body.on("click", "#btn-save-meliscms-tool-sites", function () {
		var currentTabId = activeTabId.split("_")[0],
			dataString = $(
				"#" + currentTabId + "_id_meliscms_tool_sites_edit_site form"
			).serializeArray(),
		...
		if (newEnabledModule.length) {
			var currentEnabledModule = $(
					"#" + currentTabId + "_currentEnabledModule"
				).val(),
			...
			if (moduleDiff.length > 0 && isAdmin) {
				melisCoreTool.confirm(
					...
					translations.tr_meliscms_tool_site_module_load_update_confirm.replace(
						/%s/g,
						sitesUsingModulesStr
					),
					function () {
	...
	function highlightErrs(success, errors, container) {
		if (success === 0 || success === false) {
			...
			$.each(errors, function (key, error) {
				$("#" + container + " .form-control[name='" + key + "']")
					.parents(".form-group")
					.children("label")
					.css("color", "red");
			});
			...
			if (lang != undefined) {
				...
				if (label === "") {
					...
					if (exploded[1] === "sdom") {
						if (lastChar === "*")
							label =
								$input.siblings("label").text().slice(0, -2) +
								"(" +
								exploded[0] +
								")";
						else
	...
	$body.on("click", ".btnEditSites", function () {
		...
		openSiteEditTab(
			updateSiteTitle(selId, name, siteModule, siteLang),
			tableId,
			siteModule
		);
	});
					...
					.done(function (data) {
						...
						if (data.success) {
							melisHelper.tabClose(
								siteId + "_id_meliscms_tool_sites_edit_site"
							);
	...
	$body.on("click", "#btn-new-meliscms-tool-sites", function () {
		...
		melisHelper.createModal(
			"id_meliscms_tool_sites_modal_container",
			"meliscms_tool_sites_modal_add_content",
			true,
			[],
			modalUrl,
			function () {
	...
	window.initializeStep = function () {
		owlStep = $(".sites-steps-owl").owlCarousel({
			...
			afterInit: function () {
				$(".sites-steps-owl .tool-sites_container_fixed_width")
					.find("label")
					.not(":has(input)")
					.removeClass("melis-radio-box");
				/**
		...
		switch (step) {
			...
			case "step_3":
				...
				if (isMultiLang) {
					 ...
					 */
					var multiLangFormData = getSerializedForm(
						"#step2form-multi_language"
					);
					//include the step2 data into the object
					langData = processSiteLanguage(multiLangFormData, lang);

					var multiDomainsContainer = $(
						".sites_step3-multi-domain #multi-domains_container"
					);
					var div = $("<div/>", {
						class: "form-group",
					});
					...
					$.each(multiLangFormData, function () {
						if (this.name == "site_selected_lang") {
							 ...
							 */
							var label = $("<label/>")
								.html(langData[2] + "<sup>*</sup>")
								.addClass("err_site-domain-" + this.value);
							div.append(label);
							...
							if (this.value == 2) {
								...
								if (this.value == 1) {
									domainSingleOpt =
										" (" +
										translations.tr_melis_cms_sites_tool_add_step5_single_dom_opt_1_msg +
										")";
								} else if (this.value == 3) {
									domainSingleOpt =
										" (" +
										translations.tr_melis_cms_sites_tool_add_step5_single_dom_opt_3_msg +
										")";
								}
							}
						}
					...
					domainData.isMultiDomain = false;
					//add step  2 data
					var singLangFormData = getSerializedForm(
						"#step2form-single_language"
					);
				...
				selectedLanguages =
					"- " +
					translations.tr_melis_cms_sites_tool_add_header_title_lang +
					" : " +
					lang;
			...
			case "step_5":
				 ...
				 */
				var text =
					translations.tr_melis_cms_sites_tool_add_step5_new_site_using_existing_module;
				if (newSite) {
					text =
						translations.tr_melis_cms_sites_tool_add_step5_new_site_using_new_module;
				}
				var siteSumText = text
					.replace(/%siteModule/g, siteName)
					.replace(/%siteName/g, siteLabel);
				$(".site_creation_info")
					.empty()
					.append(
						selectedLanguages,
	...
	$body.on("click", "#btn-finish-step", function (e) {
		$.ajax({
			...
			.done(function (data) {
				if (data.success) {
					...
					melisCoreTool.hideModal(
						"id_meliscms_tool_sites_modal_container_container"
					);
					...
					$.each(data.siteIds, function (i, id) {
						openSiteEditTab(
							updateSiteTitle(id, data.siteName, data.siteModuleName),
							id,
							data.siteModuleName
						);
					});
			...
			.fail(function (xhr, textStatus, errorThrown) {
				alert(
					translations.tr_melis_cms_sites_tool_add_create_site_unknown_error
				);
			});
	...
	$body.on(
		"change",
		"#is_create_new_module_for_site input[name='is_create_module']",
		function () {
			...
			if (value == "yes") {
				...
				//add required field
				addAttribute(
					step4_form.find(".form-group select[name='siteSelectModuleName']"),
					"required",
					"required"
				);
				//remove required field on create_sites_file
				removeAttribute(
					step4_form.find(".form-group input[name='create_sites_file']"),
					"required"
				);
				removeAttribute(
					step4_form.find(".form-group input[name='siteCreateModuleName']"),
					"required"
				);
			} else {
				newSite = true;
	...
	function showFormError(form, fieldNames) {
		 ...
		 */
		$("#siteAddAlert")
			.text(translations.tr_melis_cms_sites_tool_add_create_site_required_field)
			.addClass("hidden");
		...
		if (form == "#step4form_module") {
			if (!isUserSelectModuleOption) {
				$("#siteAddAlert")
					.text(
						translations.tr_melis_cms_sites_tool_add_step4_select_module_option_err
					)
					.removeClass("hidden");
				return true;
			}
		...
		if (newModuleValue != "") {
			if (/^[A-Za-z]*$/.test(newModuleValue) === false) {
				...
				$("#siteAddAlert")
					.text(
						translations.tr_melis_cms_sites_tool_add_step4_create_module_error
					)
					.removeClass("hidden");
				return true;
			}
		...
		$.each(multiDomainErr, function (lbl, value) {
			var sdomMultiLbl = $("#step3form-multi_domain")
				.find("label.err_" + lbl)
				.not(":has(input)");
		...
		if (domainNameErrCtr > 0) {
			$("#siteAddAlert")
				.text(
					translations.tr_melis_cms_sites_tool_add_step3_invalid_domain_name
				)
				.removeClass("hidden");
			return true;
		} else {
			...
			if (!$.isEmptyObject(domains)) {
				if (duplicates.length !== 0) {
					...
					$("#siteAddAlert")
						.text(
							translations.tr_melis_cms_sites_tool_add_step3_domain_unique_error
						)
						.removeClass("hidden");
					return true;
				} else {
								...
								$.when(
									$.each(data.result, function (id, val) {
										...
										$("#siteAddAlert").append(
											lang +
												" - " +
												translations.tr_melis_cms_sites_tool_add_step3_domain_error1 +
												val +
												translations.tr_melis_cms_sites_tool_add_step3_domain_error2
										);
	...
	function validatedDomainName(label, value) {
		if (
			/^(www\.)?(([a-zA-Z0-9-])+\.)?(([a-zA-Z0-9-])+\.)?(([a-zA-Z0-9-])+\.)?(([a-zA-Z0-9-])+\.)?[a-zA-Z0-9\-]{1,}(\.([a-zA-Z]{2,}))$/.test(
				value
			) === false
		) {
	...
	function openSiteEditTab(name, siteId, moduleName) {
		melisHelper.tabOpen(
			...
			siteId + "_id_meliscms_tool_sites_edit_site",
			"meliscms_tool_sites_edit_site",
			{ siteId: siteId, moduleName: moduleName },
			"id_meliscms_tool_sites",
			function () {
				$(
					"#" +
						siteId +
						"_id_meliscms_tool_sites_edit_site_header site-title-tag"
				).text(" / " + name);
			}
		);
	}
	...
	$body.on("click", "#s404_page_id_button span", function () {
		...
		melisLinkTree.createInputTreeModal(
			"#" + formId + " " + "#siteprop_s404_page_id"
		);
	});
	...
	$body.on("click", "#site_main_page_id_button span", function () {
		...
		melisLinkTree.createInputTreeModal(
			"#" + formId + " " + "#siteprop_site_main_page_id"
		);
	});
	...
	$body.on(
		"keypress",
		"#meliscms_tool_sites_domain_form  input.form-control",
		function (e) {
			return e.which !== 13;
		}
	);
	...
	$body.on(
		"click",
		"#step2form-multi_language .cb-cont input[type=checkbox]",
		function () {
			if ($(this).is(":checked")) {
				$(this).prop("checked", true);
				$(this).prev("span").find(".cbmask-inner").addClass("cb-active");
		...
		$.ajax({
			...
			.done(function (data) {
				if (data.success) {
					melisHelper.melisOkNotification(
						data.title,
						"tr_front_minify_assets_compiled_successfully"
					);
				} else {
					var errorTexts =
						"<h3>" + melisHelper.melisTranslator(data.title) + "</h3>";
					errorTexts += "<p><strong>Error: </strong>  ";
					errorTexts += "<span>" + data.message + "</span>";
					errorTexts += "</p>";

					var div =
						"<div class='melis-modaloverlay overlay-hideonclick'></div>";
					div +=
	...
	$body.on("click", ".meliscms-site-selector", function () {
		...
		$("#melis-modals-container")
			.find("#id_meliscms_page_tree_id_selector_container")
			.remove();
		meliscmsSiteSelectorInputDom = $(this)
			.parents(".input-group")
			.find("input");
		...
		$("body").on(
			"hide.bs.modal",
			"#id_meliscms_page_tree_id_selector_container",
			function () {
	...
	$body.on("click", "#selectPageId", function () {
		var tartGetId = $("#find-page-dynatree .fancytree-active")
			.parent("li")
			.attr("id");
		if (typeof tartGetId !== "undefined") {
			...
			if (meliscmsSiteSelectorInputDom.length) {
				...
				melisHelper.melisKoNotification(
					"tr_meliscms_menu_sitetree_Name",
					"tr_meliscore_error_message"
				);
			}

			// check for body tag if style = overflow: hidden
			removeParentBodyPropStyle();
		} else {
			melisHelper.melisKoNotification(
				"tr_meliscms_menu_sitetree_Name",
				"tr_meliscms_page_tree_no_selected_page"
			);
		}
	});

	$(document).on("click", "#meliscms_pagetree_modal_cancel_btn", function (e) {
		e.preventDefault();
...
window.sitesTableCallback = function () {
	 ...
	 * module is not found
	 */
	var minifBtn = $("#tableToolSites tbody tr[data-mod-found='false']").find(
		".btnMinifyAssets"
	);
	...
	minifBtn.attr(
		"title",
		translations.tr_melis_cms_minify_assets_no_module_button_title
	);
};
