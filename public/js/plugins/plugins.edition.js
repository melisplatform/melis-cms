var melisPluginEdition = (function ($, window) {
	...
	$body.on(
		"click",
		"button[id='pluginModalBtnApply'][data-page-id='" +
			melisActivePageId +
			"']",
		submitPluginForms
	);
	...
	function submitPluginForms(e) {
		...
		var $this = $(this),
			melisIdPage = $this
				.closest("#id_meliscms_plugin_modal_container")
				.data("melis-id-page"),
			melisPluginModule = $this
				.closest("#id_meliscms_plugin_modal_container")
				.data("melis-plugin-module"),
			melisPluginName = $this
				.closest("#id_meliscms_plugin_modal_container")
				.data("melis-plugin-name"),
			melisPluginId = $this
				.closest("#id_meliscms_plugin_modal_container")
				.data("melis-plugin-id"),
			melisPluginTag = $this
				.closest("#id_meliscms_plugin_modal_container")
				.data("melis-plugin-tag"),
			melisSiteModule = $this
				.closest("#id_meliscms_plugin_modal_container")
				.data("melis-site-module"),
			dataString = $this.closest(".modal-content").find("form");

		pluginHardcodedConfig = $this
			.closest("#id_meliscms_plugin_modal_container")
			.find(".plugin-hardcoded-conf")
			.text()
			.trim();
	...
	function validateModal(
		...
		$.ajax({
			type: "POST",
			url:
				"/melis/MelisCms/FrontPlugins/validatePluginModal?validate&melisSite=" +
				siteModule,
			data: datastring,
			dataType: "json",
		})
			.done(function (data) {
				if (data.success) {
					...
					if (
						$("div[data-plugin-id='" + melisPluginId + "']").parents(
							".melis-dragdropzone"
						).length
					) {
					...
					setTimeout(function () {
						...
						window.parent.melisCoreTool.hideModal(
							"id_meliscms_plugin_modal_container"
						);
					}, 300);
				} else {
	...
	function checkToolSize() {
		$(".melis-ui-outlined").each(function () {
			var $this = $(this),
				...
				totalChild =
					$this.find($(".melis-plugin-tools-box")).outerWidth() +
					$this.find($(".melis-plugin-title-box")).outerWidth();
	...
	function savePluginUpdate(data, siteModule) {
		$.ajax({
			type: "POST",
			url:
				"/melis/MelisCms/PageEdition/savePageSessionPlugin?idPage=" +
				melisActivePageId +
				"&melisSite=" +
				siteModule,
			data: data,
			...
			if (!response.success) {
				window.parent.melisHelper.melisKoNotification(
					response.textTitle,
	...
	function pluginRenderer(
		...
		var layout = $('[data-plugin-id="' + pluginId + '"]').closest(
				".melis-ui-outlined"
			),
					...
					$.each(value, function (key, val) {
						...
						if (pluginLink.length) {
							if (elType == "css") {
								...
								if (
									pluginLink.indexOf(".init") !== -1 &&
									$.inArray(pluginLink, pluginJsInitFunction) == -1
								) {
			...
			if (pluginJs[ctr] !== "undefined") {
				...
				if (ctr == pluginJs.length) {
					...
					$.each(pluginJsInitFunction, function (i, val) {
						...
						var functionName =
							urlStr.substr(0, urlStr.indexOf(".init")) + "_init";
									...
									if (reader.readyState === 4) {
										...
										if (reader.status == 200 || reader.status == 0) {
											...
											console.log(
												'Javascript Url "' +
													url +
													'" does not exist, please make sure javascript url is accessible'
											);
											// Clearing the time interval to exit
											clearInterval($tmp);
										}
	...
	function sendDragnDropList(dropzone, pageId, reference) {
		...
		var $dropzone = $("div[data-dragdropzone-id=" + dropzone + "]"),
			...
			parentDragndropContainer = $dropzone
				.parents(".melis-dragdropzone-container")
				.last(),
		...
		pluginListContainer = extractContainers(
			parentDragndropContainer,
			pageId,
			reference
		);
	...
	function extractContainers($container, pageId, reference) {
		...
		if (pluginBoxes.length) {
			...
			result.melisDragDropZoneListPlugin = pluginBoxes
				.map(function () {
					const $el = $(this);
					return {
		...
		const childContainers = $container
			.children()
			.find(".melis-dragdropzone-container")
			.filter(function () {
		...
		window.parent.melisCoreTool.confirm(
			...
			function () {
				$.ajax({
					type: "GET",
					url:
						"/melis/MelisCms/PageEdition/removePageSessionPlugin?module=" +
	...
	function createPluginModal() {
		var $this = $(this),
			...
			pluginFrontConfig = pluginContainer
				.find(".plugin-hardcoded-conf")
				.text()
				.trim(),
		...
		modalUrl =
			"/melis/MelisCms/FrontPlugins/renderPluginModal?melisSite=" + siteModule;
		...
		window.parent.melisHelper.createModal(
			zoneId,
			melisKey,
			false,
			modalParams,
				...
				setTimeout(function () {
					if ($pageIdRoot.length) {
						$pageIdRoot
							.parents(".form-group")
							.find("label")
							.addClass("d-flex flex-row justify-content-between");
					}
				}, 0);
			...
			$melisUiOutlined.each(function () {
				$(this).resizable({
					...
					stop: function (event, ui) {
						...
						var toolBox = $(ui.originalElement).children(
	...
	function getPluginData(el, percentTotalWidth) {
		...
		if (toolBox) {
			...
			$(toolBox).map(function () {
				...
				pluginList["melisIdPage"] = window.parent
					.$("#" + parent.activeTabId)
					.find(".melis-iframe")
					.data("iframe-id");
			...
			if ($(editable).length) {
				...
				$(editable).map(function () {
					...
					pluginList["tagValue"] = tinyMCE.activeEditor?.getContent({
						format: "html",
					});
				});
			}
		...
		$(".melis-dragdropzone .melis-ui-outlined").map(function () {
			...
			if ($(melisTag).length) {
				var data = $(melisTag).data(),
					pluginContainer = $(
						'div[data-melis-plugin-tag-id="' + data.pluginId + '"]'
					);
			...
			$this
				.children("[class^=plugin-width]")
				.removeClass(function (index, classes) {
					var matches = classes.match(/\bplugin-width\S+/gi);
...
var melisCmsFormHelper = (function ($, window) {
	...
	function melisMultiKoNotification(errors, closeByButtonOnly) {
		...
		$body
			.find("#id_meliscms_plugin_modal .form-group label")
			.css("color", "inherit");
						...
						try {
							$.each(error, function (key, value) {
								if (key !== "label" && key !== "form") {
									...
									if ($errMsg != "") {
										errorTexts +=
											'<span class="tets error-list"><i class="fa fa-circle"></i>' +
											$errMsg +
											"</span><br/>";
									}
								}
							...
							if (key !== "label" && key !== "form") {
								errorTexts +=
									'<span class="hoy error-list"><i class="fa fa-circle"></i>' +
									error +
									"</span>";
							}
						}
		...
		div +=
			'<div class="melis-modal-cont KOnotif page-edition-multi-ko">  <div class="modal-content error">' +
			errorTexts +
	...
	function highlightMultiErrors(success, errors) {
		...
		if (!success) {
			$.each(errors, function (key, error) {
				$body
					.find("#id_meliscms_plugin_modal .form-control[name='" + key + "']")
					.parents(".form-group")
					.find("label")
					.css("color", "red");
			});
