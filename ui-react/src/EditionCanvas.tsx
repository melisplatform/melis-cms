...
function toCell(n: DocZone, titles: Record<string, string>, depth = 0): Cell {
  return {
    id: n.id as string,
    template: n.template || '',
    ...
    cells: (n.zones || []).filter((z) => z.id).map((z) => toCell(z, titles, depth + 1)),
    // TOP-LEVEL zones only (Mantis #0011042): the sub-zones of a split layout (zone 3-1, 3-2…) carry
    // a non-empty plugin_referer too (it points at their parent), which made them look removable —
    // but they exist only as a consequence of the parent's layout and would be regenerated by it;
    // deleting one individually is meaningless, so they never get the × button.
    removable: depth === 0 && !!(n.attrs?.plugin_referer),
