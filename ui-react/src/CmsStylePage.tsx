...
function IconActive({ size = 18 }: { size?: number }) {
...
function IconInactive({ size = 18 }: { size?: number }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: 24, height: '100%', boxSizing: 'border-box', overflow: 'auto' }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        <Kpi label={t('kpi_active')} value={stats?.active ?? null} icon={<IconActive />} tint="#059669" />
        <Kpi label={t('kpi_inactive')} value={stats?.inactive ?? null} icon={<IconInactive />} tint="#dc2626" />
      </div>
